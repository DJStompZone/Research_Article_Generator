...
def extract_sentences(prompt):
    prompt_template = """You will be given a text, and your job is to create python list where each element is one of the individual sentences of the text. Only output the python list, nothing else. \nInput: {}:""".format(prompt)
    request = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": prompt_template}]
    )
...
def extract_logic(prompt):
    prompt_template = """You will be given two statements, separated by a comma. If they logically imply one another, return the string "True". If they do not logically imply one another, return the string "False." \nInput: {}:""".format(prompt)
    request = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": prompt_template}]
    )
...
def extract(prompt):
    ...
    prompt_template = """You will be given a topic, and must write an article based on it. It should be split into at least five different subsections, and each subsection should have three to four paragraphs. For each subsection, label it with a number, followed by a hyphen, not a period. \nInput: {}:""".format(prompt)
    request = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": prompt_template}]
    )
    ...
    for i in range(0,total_num-1):
        for j in range (i+1,total_num):
            ...
            logical_implication=extract_logic(str(statement_1)+","+str(statement_2))
            if logical_implication=="True":
