...
openai.api_key = "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"
...
educational = [
    ...
    "thekidshouldseethis.com", "cyberkidzgames.com"
]
computerscience = [
...
def extract_domain(prompt):
    prompt_template = """You will be given a topic, and must deterimine what topic it fits best under: news, schooling, educational, nonprofits, law, government, economics, health, science, admissions, psychological, arts, engineering, entertainment, computer science, history, countries, geography, technology, or religion. Only output the category name, uncapitalized, no other surrounding text.\nInput: {}:""".format(prompt)
...
def extract_sentences(prompt):
    prompt_template = """You will be given an input text. Extract the separate sentences from the text and return the result as a Python list (each element should only contain one sentence). \nInput: {}:""".format(prompt)
    request = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": prompt_template}]
    )
...
def extract_sentences_cited(prompt):
    prompt_template = """You have been given a Python list containing individual sentences of a passage. Find the sentences which contribute new information and return the result as a Python list (each element should only contain one sentence). \nInput: {}:""".format(prompt)
    request = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": prompt_template}]
    )
...
def similar_or_not(prompt):
    prompt_template = """You will be given two statements, separated by a comma. If they essentially have the same meaning, return the word "True", and only that word. Else, return the word "False", and only that word. \nInput: {}:""".format(prompt)
    request = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": prompt_template}]
    )
...
def find_similar_text(target_text, website_text):
    matches = difflib.get_close_matches(target_text, website_text, n=10, cutoff=0.4)
    if matches:
        number=len(matches)
        for i in range (0,number):
            if(similar_or_not(target_text+","+matches[i])=="True"):
                return matches[i]
            elif(i==number-1):
                return None
    else:
...
def extract(prompt):
    category=extract_domain(prompt)
    if(category=="news"):
    ...
    prompt_template = """You will be given a topic, and must write an article based on it. It should be split into at least five different subsections, and each subsection should have three to four paragraphs with at least 6 sentences. For each subsection, label it with a number, followed by a hyphen, not a period. Do not repeat sentences. \nInput: {}:""".format(prompt)
    request = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[{"role": "user", "content": prompt_template}]
    )
    citations=[]
    result = request['choices'][0]['message']['content'] 
    sentences=extract_sentences(result).split("', ")
    total_sentences=len(sentences)
    sentences[0]=sentences[0][2:]
    sentences[total_sentences-1]=sentences[total_sentences-1][:-2]
    for i in range(1,total_sentences-1):
        sentences[i]=sentences[i][1:]
    sentences_for_citing=extract_sentences_cited(sentences).split("', ")
    #print(len(sentences))
    #print(len(sentences_for_citing))
    total_cited=len(sentences_for_citing)
    sentences_for_citing[0]=sentences_for_citing[0][2:]
    sentences_for_citing[total_cited-1]=sentences_for_citing[total_cited-1][:-2]
    for i in range(1,total_cited-1):
        sentences_for_citing[i]=sentences_for_citing[i][1:]
    for i in sentences_for_citing:
        response=client.search(i+", "+category,num_results=5,include_domains=intended_domain)
        ...
        for n in range(websites_num):
            ...
            if(website.status_code==200):
                ...
                similar_text = find_similar_text(i, website_text)
                if similar_text:
    ...
    if(citations.count("Not found")!=0):
        ...
        for i in uncited:
            myindex=int(sentences.find(sentences_for_citing[i]))
            themessage=sentences[myindex]
            if(myindex==0):
                second_sentence=sentences[1]
                third_sentence=sentences[2]
                sentences[myindex]=extract4(str(themessage)+","+str(second_sentence)+","+str(third_sentence))
            elif(myindex==len(sentences)-1):
                second_sentence=sentences[len(sentences)-2]
                first_sentence=sentences[len(sentences)-3]
                sentences[myindex]=extract5(str(themessage)+","+str(first_sentence)+","+second_sentence)
            else:
                before=sentences[myindex-1]
                after=sentences[myindex+1]
                sentences[myindex]=extract6(str(themessage)+","+str(before)+","+str(after))
    bibliography=""
    count=1
    ...
    for k in range(0,len(citations)):
        if(citations[k]!="Not found"):
            if(citations[k] not in duplicate_prevention):
                bibliography+=str(count)+"."+str(citations[k])+"\n"
                count+=1
                duplicate_prevention.append(citations[k])

    print(bibliography)

user_input=input("Enter a topic, and an article will be generated about it, with citations: ")
extract(user_input)
